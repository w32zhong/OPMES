...
{
	struct tex_tr *n = cp_malloc(sizeof(struct tex_tr));
	n->symbol_id = s_id;
	n->token_id = t_id;
	n->n_fan = 0;
	n->node_id = 0;
...
static BOOL depth_flag[MAX_TEX_TR_DEPTH];
...
static TREE_IT_CALLBK(gen_subpath)
{
	...
	if (p->tnd.sons.now == NULL /* is leaf */ &&
		...
		sp = cp_malloc(sizeof(struct subpath));
		sp->dir[0] = '\0';
...
LIST_DEF_FREE_FUN(subpaths_free, struct subpath, ln, cp_free(p));
